(function ($) {
        ...
        // if (MyScriptData) {
        //     let objects = MyScriptData.map_data.rooms['-NSiI75fZdRvbjInjxtR'].objects;
        //
        //
        //     for (let objectKey in objects) {
        //         if (objects.hasOwnProperty(objectKey)) {
        //             let object = objects[objectKey];
        //             renderShape(object, objectKey);
        //         }
        //     }
        //
        //
        //     // room = MyScriptData.map_data.rooms['-NKJyz0m0uj0iD6P_qAP'];
        //
        //
        ...
        function updateMapData(currentid, markerData) {
            jQuery.ajax({
                url: ajaxurl,
                type: 'POST',
                data: {
                    action: 'update_map_data',
                    data: {
                        id: currentid,
                        ...markerData
                    }
        ...
        function removeMapData(currentid) {
            jQuery.ajax({
                url: ajaxurl,
                type: 'POST',
                data: {
                    action: 'remove_map_data',
                    currentid: currentid
                },
                success: function (response) {
                    if (response.success) {
        ...
        function initMap() {
            ...
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                maxZoom: 18, zoomControl: false, minZoom: 3, noWrap: true
            }).addTo(map);
        ...
        var tooltip = followcursor.bindTooltip("", {
            permanent: true, offset: [5, 25], sticky: true, className: "hints", direction: "right"
        }).addTo(map);
        ...
        function liveLocation() {
            if (navigator.geolocation) {
                ...
                navigator.geolocation.getCurrentPosition(function (position) {
                    var icon = L.icon({
                        iconUrl: 'assets/liveLocation.svg', iconSize: [24, 24], iconAnchor: [12, 12],
                    });
            ...
            $.get('https://nominatim.openstreetmap.org/search?viewbox=' + coordinates + '&format=geocodejson&limit=20&bounded=1&amenity=' + locationtype + '&exclude_place_ids=' + JSON.stringify(place_ids), function (data) {
                ...
                data.features.forEach(function (place) {
                    ...
                    var marker = L.marker([place.geometry.coordinates[1], place.geometry.coordinates[0]], {
                        icon: marker_icon, pane: "overlayPane", interactive: true
                    }).addTo(map);
        ...
        function saveNearby() {
            ...
            inst.marker.bindTooltip('<h1>' + inst.name + '</h1><div class="shape-data"><h3><img src="assets/marker-small-icon.svg">' + inst.lat.toFixed(5) + ', ' + inst.lng.toFixed(5) + '</h3></div><br><div class="arrow-down"></div>', {
                permanent: false,
                direction: "top",
                interactive: false,
                bubblingMouseEvents: false,
        ...
        function cancelForm() {
            ...
            if (inst.type == "area") {
                ...
                updateMapData(currentid, {
                    area: inst.area, distance: inst.distance, name: inst.name, desc: inst.desc, completed: true
                })
        ...
        map.on('pm:create', e => {
            ...
            var centermarker = L.marker(e.layer.getBounds().getCenter(), {
                zIndexOffset: 9999, interactive: false, pane: "overlayPane"
            });
        ...
        function startDrawing(lat, lng) {
            ...
            objects.push({
                id: currentid, line: line, session: session, local: true, completed: true, type: "draw"
            });
            line.addTo(map);
        ...
        function createMarker(lat, lng, user) {
            if (markerson) {
                ...
                var marker = L.marker([lat, lng], {
                    icon: marker_icon, direction: "top", interactive: true, pane: "overlayPane"
                });
        ...
        map.addEventListener('mousemove', (event) => {
            ...
            if (typeof lat != undefined && typeof lng != undefined) {
                if (!dragging) {
                    ...
                    //     lat: lat, lng: lng, view: [map.getBounds().getCenter().lat, map.getBounds().getCenter().lng]
                    // });
                } else {
        ...
        map.addEventListener('zoom', (event) => {
            ...
            // db.ref('rooms/' + room + '/users/' + user.uid).update({
            //     view: [map.getBounds().getCenter().lat, map.getBounds().getCenter().lng], zoom: map.getZoom()
            // });
        ...
        function createMap() {
            ...
            //     name: "New map", description: "Map description"
            // });
        ...
        function addShapeInfo(snapshot, key) {
            if (snapshot.type != "draw" && snapshot.type != "marker") {
                ...
                if (inst.type == "line") {
                    centermarker.bindTooltip('<h1>' + inst.name + '</h1><h2>' + inst.desc + '</h2><div class="shape-data"><h3><img src="assets/distance-icon.svg">' + inst.distance + ' km</h3></div><div class="arrow-down"></div>', {
                        permanent: false,
                        direction: "top",
                        interactive: false,
                        closeOnClick: false,
                    ...
                    centermarker.bindTooltip('<h1>' + inst.name + '</h1><h2>' + inst.desc + '</h2><div class="shape-data"><h3><img src="assets/area-icon.svg">' + inst.area + ' km&sup2;</h3></div><div class="shape-data"><h3><img src="assets/perimeter-icon.svg">' + inst.distance + ' km</h3></div><div class="arrow-down"></div>', {
                        permanent: false,
                        direction: "top",
                        interactive: false,
                        bubblingMouseEvents: false,
            ...
            if (snapshot.type == "draw" || snapshot.type == "line" || snapshot.type == "area") {
                ...
                if (objects.filter(function (result) {
                    ...
                    var marker = L.marker([snapshot.lat, snapshot.lng], {
                        icon: marker_icon, interactive: true, direction: "top", pane: "overlayPane"
                    });

                    // Create the popup that shows data about the marker
                    marker.bindTooltip('<h1>' + snapshot.name + '</h1><h2>' + snapshot.desc + '</h2><div class="shape-data"><h3><img src="assets/marker-small-icon.svg">' + snapshot.lat.toFixed(5) + ', ' + snapshot.lng.toFixed(5) + '</h3></div><div class="arrow-down"></div>', {
                        permanent: false,
                        direction: "top",
                        className: "create-shape-flow tooltip-off",
                        interactive: false,
